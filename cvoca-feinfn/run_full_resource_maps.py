...
def apply_final_logits(trainer: DecoupledLongTailTrainer, logits: torch.Tensor) -> torch.Tensor:
...
@torch.no_grad()
def evaluate_for_metrics(trainer: DecoupledLongTailTrainer, loader, num_classes: int) -> Dict[str, Any]:
    trainer.model.eval()
    preds = []
    labels = []
    total_loss = 0.0
...
def build_trainer(cfg: Dict[str, Any], bundle, model: SPARCNet) -> DecoupledLongTailTrainer:
    ...
    return DecoupledLongTailTrainer(
        model,
        classification_loss,
