...
SELECTED_GPU_INDEX: Optional[int] = 1
...
CONFIG: Dict[str, Any] = {
    ...
    "split": {
        ...
        "train_ratio": 0.003,  # 训练集比例；仅在 train_mode='ratio' 时生效。
    ...
    "model": {
        ...
        "freeze_source_classifier_in_stage2": False,
    },
    "loss": {
        "total_epochs": 150,
        "max_margin": 0.3,
    ...
    "trainer": {
        ...
        "stage2_use_balanced_loader": False,
        "aux_mid_weight": 0.3,
        "aux_early_weight": 0.15,
        "contrastive_weight": 0.1,
        "stage1_head_regularization_weight": 0.02,
        "stage2_head_regularization_weight": 0.04,
        "amp": False,
...
CONFIG["dataset"]["ablation"] = {
    ...
    "use_phase_amplitude_fusion_backbone": True,  # 创新点1总开关：复值感知空频融合主干。关闭后改用普通残差CNN backbone。
    "use_dynamic_prototype_head": False,  # 创新点2总开关：动量双原型关系分类头。关闭后改用普通 cosine baseline head。
    "use_decoupled_training_engine": True,  # 创新点3总开关：两阶段解耦训练引擎。关闭后改为单阶段训练。
    "use_adaptive_posthoc_calibration": True,  # 创新点4总开关：事后校准模块。关闭后不进行 stage3 calibration。
    ...
    "training_detail": {
        ...
        "use_contrastive_loss": True,  # 对比学习损失：控制表征拉近/拉远约束。
    },
}
...
def get_ablation_switches(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    backbone_on = innovation["use_phase_amplitude_fusion_backbone"]
    prototype_head_on = innovation["use_dynamic_prototype_head"]
    training_engine_on = innovation["use_decoupled_training_engine"]
    calibration_on = innovation["use_adaptive_posthoc_calibration"]
    ...
    return {
        ...
        "use_sfid": backbone_on and detail["backbone_detail"]["use_tsfi"],
        ...
        "use_decoupled_training": training_engine_on,
...
def build_effective_stage_plan(config: Dict[str, Any], ablation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not ablation["use_decoupled_training"]:
...
def print_ablation_plan(config: Dict[str, Any]) -> None:
    ...
    print(
        format_key_value_block(
            [
                ("Innovation1 total module", innovation["use_phase_amplitude_fusion_backbone"]),
                ("Innovation2 total module", innovation["use_dynamic_prototype_head"]),
                ("Innovation3 total module", innovation["use_decoupled_training_engine"]),
                ("Innovation4 total module", innovation["use_adaptive_posthoc_calibration"]),
            ]
        )
    )
    print("")
...
def main() -> None:
    ...
    val_eval = trainer.evaluate(bundle.val_loader)
