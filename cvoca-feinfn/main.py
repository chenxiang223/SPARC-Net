...
CONFIG["dataset"]["ablation"] = {
    ...
    "use_dynamic_prototype_head": True,  # 创新点2总开关：动量双原型关系分类头。关闭后改用普通 cosine baseline head。
    "use_decoupled_training_engine": True,  # 创新点3总开关：两阶段解耦训练引擎。关闭后改为单阶段训练。
    "use_adaptive_posthoc_calibration": False,  # 创新点4总开关：事后校准模块。关闭后不进行 stage3 calibration。
