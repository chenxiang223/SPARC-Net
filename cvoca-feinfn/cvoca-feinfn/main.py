...
CONFIG["dataset"]["ablation"] = {
    ...
    "use_mrpc_head": True,  # 创新点2总开关：MRPC Head。关闭后改用普通 cosine baseline head。
    "use_staged_training_engine": True,  # 创新点3总开关：主分支优先分阶段训练引擎。关闭后改为单阶段训练。
    "use_rtpc_calibration": False,  # 创新点4总开关：RTPC 模块。关闭后不进行 stage3 calibration。

    # 二级子消融开关：
    # 这些开关仅在对应的一级总开关为 True 时生效，用于分析模块内部子机制。
    "backbone_detail": {
        "use_complex_attention": True,  # APCR 相幅通道重标定：控制幅值/相位联合重标定。
